...
describe('on signup success saga', () => {
       it('should trigger on SIGN_UP_SUCCESS', () => {
         ...
         expect(generator.next().value).toEqual(
           takeLatest(UserActionTypes.SIGN_UP_SUCCESS, signUptoSignIn)
         );
       });
     });
     ...
     describe('on google sign in start saga', () => {
       it('should trigger on GOOGLE_SIGN_IN_START', () => {
         const generator = onGooglesignin();
         expect(generator.next().value).toEqual(
           takeLatest(UserActionTypes.GOOGLE_SIGN_IN_START, signInWithGooglepop)
         );
       });
     });
     ...
     describe('on sign in after sign up saga', () => {
       it('should fire getSnapshotFromUserAuth', () => {
         ...
         const generator = signUptoSignIn(mockAction);
         expect(generator.next().value).toEqual(
        onUserAuthchange(mockUser, mockAdditionalData)
         );
       });
     });
     ...
     describe('get snapshot from userAuth', () => {
       const mockUserAuth = {};
       const mockAdditionalData = {};
       const generator = onUserAuthchange(mockUserAuth, mockAdditionalData);
     
       expect(generator.next().value).toEqual(
         call(createDocument, mockUserAuth, mockAdditionalData)
       );
     });